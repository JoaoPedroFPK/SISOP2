...
void PrimaryServer::run(int port) {
    DEBUG_PRINTF("aqui\n");
    if (port > 0) {
        serverPort = port;
    }
    ...
    while (running.load()) {
        ...
        if (clientSocket >= 0) {
            std::thread clientThread(&PrimaryServer::handleClient, this, clientSocket);
            clientThread.detach();
        }
    }
}
...
void PrimaryServer::handleClient(int clientSocket) {
    DEBUG_PRINTF("aqui");
    Message msg;
    Result result;
    std::string username;

    result = protocol.receiveMessage(clientSocket, msg);
    if (msg.command == Command::LOGIN) {
        // TODO: handle frontend request
    }
    else if (msg.command == Command::REGISTER_SERVER) {
        //
    }
    else if (msg.command == Command::REQUEST_STATE) {
